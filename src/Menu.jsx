   ...
   function changefilter(fil){
    ...
    return (   
        ...
        <div className="bg-slate-800 text-white py-10 items-center">
            ...
            {return (
                <button className="font-Montserrat text-xl capitalize " onClick={()=>changefilter(category) }>{category===filters ? <span className="underline underline-offset-8">{category}</span> : <span>{category}</span>}</button>
            )}
            )}
            </header>
